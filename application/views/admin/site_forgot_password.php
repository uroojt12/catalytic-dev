        ...
        <div class="form-group">
            <div class="col-md-10">
                <div class="form-group">
                    ...
                    <div class="col-md-3">
                        <label for="left_tagline" class="control-label">Link Tagline<span class="symbol required">*</span></label>
                        <input type="text" name="left_tagline" value="<?= $row['left_tagline'] ?>" class="form-control" required>
                    </div>
                    <div class="col-md-3">
                        <label for="left_tagline_link_text" class="control-label">Link Text<span class="symbol required">*</span></label>
                        <input type="text" name="left_tagline_link_text" value="<?= $row['left_tagline_link_text'] ?>" class="form-control" required>
                    </div>
                    <div class="col-md-3">
                        <label for="link_url" class="control-label">Link<span class="symbol required">*</span></label>
                        <select name="link_url" id="link_url" class="form-control" required>
                            <option value=''>-- Select --</option>
                            <?php $pages = get_pages();
                            foreach ($pages as $index => $page) { ?>
                                <option value="<?= $index ?>" <?= ($row['link_url'] == $index) ? 'selected' : '' ?>> <?= $page ?></option>
                            <?php
                            }
                            ?>
                        </select>
